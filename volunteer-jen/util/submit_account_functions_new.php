<?php
	...
	**/
	function search_for_member_by_email($email, $fname, $lname){
		global $conn;
		$result = mysqli_query($conn, "SELECT memberId FROM Member WHERE email = '$email' and firstName='$fname' and lastName='$lname'") or die ('search for member by email Error: '.mysqli_error($conn));
	...
	**/
	function search_for_member_by_email_in_security($email){
		global $conn;
		$result = mysqli_query($conn, "SELECT memberId FROM Security WHERE emailAddress = '$email'") or die ('Error in search for member in security: '.mysqli_error($conn));
		$memberId = mysqli_fetch_array($result);
		if(!$memberId){
			return -1;
		}else{
	...
	**/
	function search_for_member_by_name($firstName, $lastName, $zip){ 
		global $conn;
		$result = mysqli_query($conn, "SELECT memberId FROM Member WHERE firstName = '$firstName' AND lastName = '$lastName' AND zip = '$zip'") or die ('Error in search for member by name: '.mysqli_error ($conn));
		$memberId = mysqli_fetch_array($result);
		if(!$memberId){
			return -1;
		}else{
	...
	**/
	function search_for_password($memberId){
		global $conn;
		$result = mysqli_query($conn, "SELECT memberId FROM Security WHERE memberId = $memberId") or die ('Error in search for password: '.mysqli_error ($conn));		
	...
	**/
	function how_many_by_name($firstName, $lastName, $zip){
		global $conn;
		$result = mysqli_query($conn, "SELECT memberId FROM Member WHERE firstName = '$firstName' AND lastName = '$lastName' AND zip = '$zip'") or die ('Error in how many by namE: '.mysqli_error ($conn));
		$counter = 0;
	...
	**/
	function create_new_member($firstName, $lastName, $zip, $email){
		...
		mysqli_query($conn, "INSERT INTO Member 
  		...
  		or die('Error in create new member: '.mysqli_error($conn));  
	...
	**/
	function add_member_to_security($email, $password, $memberId) {
		...
		mysqli_query($conn, "INSERT INTO Security (emailAddress, password, memberId) VALUES ('$email', '$password', $memberId)") or die('Error in add member to security: '.mysqli_error($conn));
	}
	...
	**/
	function update_member($memberId, $firstName, $lastName, $zip){
		global $conn;
		mysqli_query($conn, "UPDATE Member SET firstName = '$firstName', lastName = '$lastName', zip = '$zip' WHERE memberId = $memberId") or die('Error in update member: '.mysqli_error($conn));
	}
	...
	**/
	function update_member_email($memberId, $email){
		global $conn;
		mysqli_query($conn, "UPDATE Member SET email = '$email' WHERE memberId = $memberId") or die('Error in update member email: '.mysqli_error($conn));
	}

<?php
	...
	**/
	...
	function display_user_total_vol_hours($memberId){
		global $conn;
		$result = mysqli_query($conn, "SELECT SUM(numHours) FROM VolunteerHours WHERE memberId = $memberId") or die ("Error: ".mysqli_error($conn));
	...
	**/
	function display_user_total_yearly_hours($memberId){
		...
		$result = mysqli_query($conn, "SELECT SUM(numHours) FROM VolunteerHours WHERE memberId = $memberId AND DATE_FORMAT(volDate, '%Y') = $current_year") or die ("Error: ".mysqli_error($conn));
	...
	**/
	function display_user_hours($memberId){	
		global $conn;
		$result = mysqli_query($conn, "SELECT hoursId, volDate, projectName, activity, location, numHours FROM VolunteerHours, Projects WHERE Projects.projectId = VolunteerHours.projectId AND VolunteerHours.memberId = $memberId ORDER BY volDate DESC") or die ("Error: ".mysqli_error($conn));
